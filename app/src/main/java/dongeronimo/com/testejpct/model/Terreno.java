...
public class Terreno {
    ...
    private int bmpLargura;
    private int bmpAltura;
    ...
    public Terreno(Bitmap heightmap, Mundo mundo, float seaLevel){
        ...
        bmpAltura = heightmapBitmap.getHeight();
        ...
        for(int ln = 0; ln <bmpAltura; ln++){
            for (int col=0; col<bmpLargura; col++){
                ...
                final int colorRawData = heightmapBitmap.getPixel(col, ln);
    ...
    private void buildTerrain(){
        ...
        //aloca as arrays que eventualmente serão passadas pro opengl
        ...
        //Essa estrutura aqui é pra auxiliar os algoritmos
        List<Pair<Integer, Integer>> xyMap = new ArrayList<>();
        //no momento tdas as uvs serao todas 0, pq nao as uso
        for(int i=0; i<uvs.length;i++)
            uvs[i] = 0.0f;
        int coordIndex = 0;
        int pointId = 0;
        long tBuildVertData = System.currentTimeMillis();
        //essa estrutura de dados é pra ajudar no algoritmo dos cálculos das
        //normais
        class DataStruct{
        ...
        String fragShaderSrc = Loader.loadTextFile(mundo.getContext().getResources().openRawResource(R.raw.teste_fragment_shader));
